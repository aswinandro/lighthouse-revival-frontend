      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            value: `$${reports.reduce((sum, r) => sum + r.expenses, 0).toLocaleString()}`,
            icon: TrendingUp,
            color: "bg-red-500",
          },
          {
            label: "Net Balance",
            value: `$${reports.reduce((sum, r) => sum + r.netBalance, 0).toLocaleString()}`,
            icon: DollarSign,
            color: "bg-blue-500",
          },
          {
            label: "Total Attendance",
            value: reports.reduce((sum, r) => sum + r.attendance, 0).toLocaleString(),
            icon: Users,
            color: "bg-purple-500",
          },
        ].map((stat, index) => {
            ...
            <TableBody>
              {reports.map((report) => (
                <TableRow key={report.id}>
                  ...
                  <TableCell className="text-green-600 font-medium">${report.totalIncome.toLocaleString()}</TableCell>
                  <TableCell className="text-red-600 font-medium">${report.expenses.toLocaleString()}</TableCell>
                  <TableCell className="font-bold">${report.netBalance.toLocaleString()}</TableCell>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1">
                                  ...
                                  <p className="text-lg font-medium text-green-600">${report.tithe.toLocaleString()}</p>
                                </div>
                                <div className="space-y-1">
                                  <Label className="text-sm text-muted-foreground">Offering</Label>
                                  <p className="text-lg font-medium text-green-600">
                                    ${report.offering.toLocaleString()}
                                  </p>
                                </div>
                                <div className="space-y-1">
                                  <Label className="text-sm text-muted-foreground">Special Offering</Label>
                                  <p className="text-lg font-medium text-green-600">
                                    ${report.specialOffering.toLocaleString()}
                                  </p>
                                </div>
                                <div className="space-y-1">
                              ...
                              <div className="pt-2 border-t">
                                <div className="space-y-1">
                                  ...
                                  <p className="text-xl font-bold text-red-600">${report.expenses.toLocaleString()}</p>
                                </div>
                              </div>
                              <div className="pt-2 border-t">
                                <div className="space-y-1">
